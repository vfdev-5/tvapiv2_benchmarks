...
def gen_inputs():
    ...
    for make, shape, device, fn_name, threads in product(makers, shapes, devices, fns, threads):
        ...
        args = (45.0, [0.2, 0.1], 1.0, [0.0, 0.0], )

        fn = getattr(F_v2, fn_name)
        yield f"{fn_name.capitalize()} {device} {t1.dtype}", str(tuple(shape)), threads, "v2", fn, t1, *args

        fn = getattr(F_stable, fn_name)
        yield f"{fn_name.capitalize()} {device} {t1.dtype}", str(tuple(shape)), threads, "stable", fn, t1, *args
